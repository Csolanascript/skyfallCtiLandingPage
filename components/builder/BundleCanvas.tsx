...
export default function BundleCanvas() {
  ...
  // Ref to track whether a card click handled the pending edge (avoid opening radial)
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === "Escape") setPendingEdge(null); };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, []);

  // ── Start edge from card handle
  const handleStartEdge = useCallback((uid: string, handlePos: { x: number; y: number }) => {
    setPendingEdge({ sourceUid: uid, screenX: handlePos.x, screenY: handlePos.y });
  }, []);

  // ── Complete edge on card click (when pending)
  const handleCardClick = useCallback((uid: string, cardPos: { x: number; y: number }) => {
    if (!pendingEdge || pendingEdge.sourceUid === uid) {
      setPendingEdge(null);
      return;
    }
    cardHandled.current = true;
    setRelDropdown({
      srcUid: pendingEdge.sourceUid,
      tgtUid: uid,
      x: cardPos.x,
      y: cardPos.y,
    });
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (radialMenu) return; // radial has its own backdrop
        if (!pendingEdge) return;
        if (cardHandled.current) { cardHandled.current = false; return; }
        // Released on empty canvas space → open radial menu
        const srcEntity = entities.find((en) => en.uid === pendingEdge.sourceUid);
        if (!srcEntity) { setPendingEdge(null); return; }
        // Only open if there are valid targets
        const items = getRadialItems(srcEntity.type);
        if (items.length === 0) { setPendingEdge(null); return; }
        const rect = canvasRef.current!.getBoundingClientRect();
        setRadialMenu({
          srcUid:  pendingEdge.sourceUid,
          srcType: srcEntity.type,
      ...
      {entities.length === 0 && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 9, letterSpacing: "0.2em", color: C.muted }}>
              DRAG ENTITIES FROM THE PALETTE
            </div>
            <div style={{ fontSize: 8, letterSpacing: "0.15em", color: C.muted, marginTop: 6 }}>
              OR CLICK TO PLACE
            </div>
          </div>
        </div>
        ...
        {relationships.map((r) => {
          ...
          return (
            <g key={r.uid}>
              ...
              {/* Relationship label */}
              <text
                style={{ fontSize: 7, fontFamily: C.mono, fill: C.muted, fillOpacity: 0.7 }}
                dy={-4}
              >
